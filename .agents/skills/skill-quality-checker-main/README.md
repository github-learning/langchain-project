...
```bash
node scripts/check_quality.mjs --scan-dir ~/.claude/skills/
```

### 评估单个 skill

```bash
node scripts/check_quality.mjs --skill ~/.claude/skills/my-skill/
```
...
```bash
# Markdown 格式
node scripts/check_quality.mjs --scan-dir ~/.claude/skills/ --format markdown --output report.md

# JSON 格式
node scripts/check_quality.mjs --scan-dir ~/.claude/skills/ --format json --output report.json
```
