...
// 向量化存储： 把向量和文档一起存进 MemoryVectorStore（内存向量库）：
const vectorStore = await MemoryVectorStore.fromDocuments(
  documents,
  embeddings,
);
// 产物是：一个可检索的向量库 vectorStore
// 从向量数据库中拿一个检索器 retriever，并设置要取多少个结果（k）：
...
for (const question of questions) {
  ...
  // 显式展示：问题（query）也会被向量化成 number[]，再去向量库做相似度检索
  const queryVector = await embeddings.embedQuery(question);
  console.log("\n【问题向量（Embedding）摘要】");
  console.log('=='.repeat(80),  queryVector);
  // 使用 retriever 获取文档
  const retrievedDocs = await retriever.invoke(question);

  console.log('%c [ retrievedDocs ]-109', 'font-size:16px; background:pink; color:#bf2c9f;', retrievedDocs)
  
  // 使用 similaritySearchWithScore 获取相似度评分
  const scoredResults = await vectorStore.similaritySearchWithScore(question, 3);

  console.log('%c [ scoredResults ]-115', 'font-size:16px; background:pink; color:#bf2c9f;', scoredResults)
  // 打印用到的文档和相似度评分
  console.log("\n【检索到的文档及相似度评分】");
...
console.log('%c [ context+++++++++++++++++ ]-135', 'font-size:16px; background:pink; color:#bf2c9f;', context)
